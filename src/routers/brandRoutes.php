...
Route::group(['prefix' => config('sap.custom_admin_path'),'middleware' => ['web', 'auth_admin', 'can:Access Admin Panel']], function () {
    Route::group(['prefix' => 'brand', 'namespace' => config('sap.controller_namespace'). '\Admin'], function () {
        ...
        Route::match(['get', 'head'], 'create', 'BrandController@create')->name('brand.create');
        Route::match(['post'], 'store', 'BrandController@store')->name('brand.store');
        ...
        Route::match(['delete'], '{model}/delete', 'BrandController@destroy')->name('brand.destroy');
    });
});
