...
Route::group(['prefix' => config('sap.custom_admin_path'),'middleware' => ['web', 'auth_admin', 'can:Access Admin Panel']], function () {
    Route::group(['prefix' => 'file', 'namespace' => config('sap.controller_namespace') . '\Admin'], function () {
        ...
        Route::match(['put'], '{path?}/directories/make', 'FileController@make')->name('folder.make');
        Route::match(['put'], '{path?}/directories/rename', 'FileController@change')->name('folder.change');
        Route::match(['put'], '{path?}/directories/clone', 'FileController@clone')->name('folder.clone');
        Route::match(['delete'], '{path?}/directories/remove', 'FileController@remove')->name('folder.remove');
        ...
        Route::match(['post'], '{path?}/upload', 'FileController@upload')->name('file.upload');
